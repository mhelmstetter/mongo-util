...
public class ShardedDiffTask implements Callable<DiffResult> {
    ...
    private BsonDocument minKey(RawBsonDocument chunk) {
        BsonDocument minVal = (BsonDocument) chunk.get("min");
        return minVal;
    }

    private BsonDocument maxKey(RawBsonDocument chunk) {
    ...
    private long getChunkSize(RawBsonDocument chunk) {
		...
		chunkSizeCmdParams.put("min", minKey(chunk));
		chunkSizeCmdParams.put("max", maxKey(chunk));
