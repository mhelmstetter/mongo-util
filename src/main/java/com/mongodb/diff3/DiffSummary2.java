...
public class DiffSummary2 {
    private static Logger logger = LoggerFactory.getLogger(DiffSummary2.class);
    ...
    public DiffSummary2(long totalDocs, long totalSize) {
    ...
    public void updateInitTask(DiffResult result) {
        ...
        String chunkId = result.unitString();
    ...
    public synchronized void updateRetryTask(DiffResult result) {
        ...
        synchronized (m) {
            ...
            if (failures > 0){
                ...
                cr.setDestOnly(result.getKeysOnlyOnDest());
                cr.setSourceOnly(result.getKeysOnlyOnSource());
            } else {
                cr.setStatus(DiffStatus.SUCCEEDED);
                cr.removeMismatches();
                cr.removeSourceOnly();
    ...
    private ChunkResult findChunkResult(DiffResult result) {
        ...
        cr = nsMap.get(result.unitString());
        if (cr == null) {
            logger.error("Could not find chunk ({}) in summary map", result.unitString());
            throw new RuntimeException("Could not update chunk: " + result.unitString()
